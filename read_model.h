    ...
    while (!feof(f_in))
    {
          ...
          line[3] == 'N' && line[4] == 'T' && line[5] == 'S' && line[6] == ' ')
      {
        ...
        for (int t = 0; t < cell_num; ++t)
        {
          ...
          {
            if (std::is_same<U, size_t>::value)
              EXITIF(fscanf(f_in, "%*d%zu%zu%zu%zu%zu%zu", &cells(t, 0), &cells(t, 1),
                            &cells(t, 2), &cells(t, 3), &cells(t, 4), &cells(t, 5)) < 6,
                     "CELLS format error")
            else
              EXITIF(fscanf(f_in, "%*d%d%d%d%d%d%d", &cells(t, 0), &cells(t, 1),
                            &cells(t, 2), &cells(t, 3), &cells(t, 4), &cells(t, 5)) < 6,
                     "CELLS format error");
